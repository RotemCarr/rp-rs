...
fn ppb() -> RegisterBlock {
    let base = if is_secure() { PPB_BASE_SECURE } else { PPB_BASE_NON_SECURE };
    RegisterBlock::new(base)
}
...
/// Data memory barrier (required for synchronization after VTOR writes).
#[inline(always)]
unsafe fn dmb() {
...
pub fn vtor_read() -> usize {
    unsafe { ppb().read(VTOR_OFFSET) as usize }
}

/// Write a new base address to VTOR.
///
/// Automatically handles Secure vs. non-secure PPB, required alignment masking,
/// and data/instruction barriers.
///
/// # Safety
///
/// If `addr` does not point to a valid vector table the CPU will fault.
#[inline(always)]
pub unsafe fn vtor_write(addr: u32) {
    ppb().write(VTOR_OFFSET, addr & !0x7F);
    dmb();
    isb();
}

/// Install an interrupt handler at runtime.
///
/// # Safety
///
/// Directly edits the RAM vector table — an invalid handler address causes an irrecoverable fault.
...
/// Copy the flash vector table into RAM (required before installing dynamic handlers).
...
/// Enable a hardware interrupt in the NVIC.
///
/// # Safety
///
/// The caller must ensure the vector table has been relocated to RAM before calling.
pub unsafe fn nvic_enable(irq: Interrupt) {
    let irq_num = interrupt_num(irq);
    let iser_offset = NVIC_ISER0 + 4 * (irq_num / 32);
    let ppb = RegisterBlock::new(PPB_BASE_SECURE);
    ppb.write(iser_offset, 1u32 << (irq_num % 32));
}
